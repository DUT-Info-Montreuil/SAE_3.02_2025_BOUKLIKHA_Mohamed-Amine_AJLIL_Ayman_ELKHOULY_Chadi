...
class ModeleAdmin extends Connexion {
    ...
    public function getDemandesAssociation() {
        $req = self::$bdd->prepare("SELECT d.*, u.nom AS nom_utilisateur, u.prenom AS prenom_utilisateur FROM DemandeAssociation d JOIN Utilisateur u ON d.id_utilisateur = u.id_utilisateur");
        $req->execute();
        return $req->fetchAll();
    }
